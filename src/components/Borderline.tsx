...
class GlobalCorners {
	...
	isSharp(point: Point, sharpness: CornerSharpness) {
		if (point[0] === this.topRight[0] && point[1] === this.topRight[1]) {
			return sharpness.topRight
		}
		
		if (point[0] === this.topLeft[0] && point[1] === this.topLeft[1]) {
			return sharpness.topLeft
		}
		
		if (point[0] === this.bottomRight[0] && point[1] === this.bottomRight[1]) {
			return sharpness.bottomRight
		}
		
		if (point[0] === this.bottomLeft[0] && point[1] === this.bottomLeft[1]) {
...
const Borderline = ({
	...
	useLayoutEffect(() => {
		const calculatePath = () => {
			if (ref.current) {
				const { left: parentLeft, top: parentTop } = ref.current.getBoundingClientRect()
				const polygons: number[][][][] = []
				
				// const globalCorners = new GlobalCorners()
				
				let globalTopPoints: Point[] = [[Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY]]
				let globalBottomPoints: Point[] = [[Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY]]

				// Collect rectangles from child elements
				for (const child of Array.from(ref.current.children)) {
					const { left: globalLeft, top: globalTop, right: globalRight, bottom: globalBottom } = child.getBoundingClientRect()
					...
					if (top < globalTopPoints[0][1]) {
						globalTopPoints = [[left, top], [right, top]]
					...
					if (bottom > globalBottomPoints[0][1]) {
						globalBottomPoints = [[left, bottom], [right, bottom]]
				...
				const globalCorners = new GlobalCorners(globalTopPoints, globalBottomPoints)
					...
					for (const points of polygon) {
						...
						for (let i = 0; i < points.length; i++) {
							...
							if (cornerRadius > 0) {
								...
								const startPoint = [current[0] - (dx1 * radius1) / len1, current[1] - (dy1 * radius1) / len1]

								const endPoint = [current[0] + (dx2 * radius2) / len2, current[1] + (dy2 * radius2) / len2]

								// Calculate control points using their respective radii
								const control1 = [current[0] - (dx1 * radius1 * controlRatio) / len1, current[1] - (dy1 * radius1 * controlRatio) / len1]

								const control2 = [current[0] + (dx2 * radius2 * controlRatio) / len2, current[1] + (dy2 * radius2 * controlRatio) / len2]
						...
						if (corners.length > 0) {
							...
							pathComponents.push(`M ${corners[0].start[0]} ${corners[0].start[1]}`)
							...
							for (let i = 0; i < corners.length; i++) {
								...
								if (corner.control1 && corner.control2) {
									...
									pathComponents.push(`L ${corner.start[0]} ${corner.start[1]}`)
								}
								
								// Draw line to the next corner's start point (wrapping to first corner)
								const nextCorner = corners[(i + 1) % corners.length]
								pathComponents.push(`L ${nextCorner.start[0]} ${nextCorner.start[1]}`)
							}
						}
